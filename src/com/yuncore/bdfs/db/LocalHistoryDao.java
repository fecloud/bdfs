...
public class LocalHistoryDao extends BaseDao {
	...
	public synchronized List<LocalHistory> getHistory() {
		return getHistory(0);
	}

	public synchronized List<LocalHistory> getHistory(int count) {
		...
		final List<LocalHistory> list = new ArrayList<LocalHistory>();
		try {
			final Connection connection = getDB();
			final PreparedStatement prepareStatement = connection
					.prepareStatement(sql);
