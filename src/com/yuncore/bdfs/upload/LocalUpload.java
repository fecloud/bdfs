package com.yuncore.bdfs.upload;
...
import com.yuncore.bdfs.entity.MkDirResult;
...
public class LocalUpload extends Thread implements OutputDataListener {
	...
	private boolean checkLocalFile(BDFSFile file) {
		final String localpath = String.format("%s/%s", root, file.getAbsolutePath());
	...
	private boolean norMalFileContext(BDFSFile file) throws ApiException {
		...
		try {
			...
			StatusMent.setProperty(StatusMent.key_upload_size, 0);
			throw new ApiException(String.format("uploadFileContext file:%s error", file.getAbsolutePath()), e);
		}
	}
	...
	private boolean mkdirCloud(BDFSFile file) throws ApiException {
		try {
			...
			if (null != mkdir) {
				...
				Log.w(TAG, "mkdirCloud " + file.getAbsolutePath() + " error:" + mkdir.getStatus());
			}
		} catch (ApiException e) {
