 ...
 *  本地扫描上传
 */
package com.yuncore.bdfs.server.web;
...
public class BDFSServlet extends HttpServlet {
		...
		if (dir.exists()) {
			...
			if (null != list && list.length > 0) {
				...
				for (File s : list) {
					...
					once.put("time", DateUtil.formatTime(s.lastModified()));
					array.put(once);
					s.delete();
				}
	...
	private final static String execmd(String cmd) throws Exception {
